                ...
                <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto items-center mb-24">
                    {plans.map((plan, i) => (
                        ...
                        >
                            ...
                            <Button onClick={() => onNavigate('login')} variant={plan.popular ? 'dark' : 'outline'} className={`w-full text-lg border-2 ${!plan.popular ? 'dark:border-white dark:text-white dark:hover:bg-white dark:hover:text-black' : ''}`} size="lg">
                                Choose {plan.name}
                            </Button>
                        </motion.div>
                    ))}
